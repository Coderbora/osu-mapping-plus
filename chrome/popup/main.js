...
$('#backup').click(async function (event) {
    ...
    download(`backup_osump_${Date.now()}`, JSON.stringify({
        notifications: await new Promise(resolve => chrome.runtime.sendMessage({ q: "notifications" }, resolve)),
        subscriptions: await new Promise(resolve => chrome.runtime.sendMessage({ q: "subscriptions" }, resolve))
    }))
})
        ...
        reader.onload = function (evt) {
            try {
                ...
                if(jsonTest.hasOwnProperty("notifications") && jsonTest.hasOwnProperty("subscriptions")) {
                    chrome.runtime.sendMessage({ q: "updateStore", store: evt.target.result})
