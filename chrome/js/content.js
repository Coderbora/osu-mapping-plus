...
const reloadSubscriptions = async () => { subscriptions = await new Promise(resolve => chrome.runtime.sendMessage({q: "subscriptions"},response => resolve(response))) }
const reloadNotifications = async () => { notifications = await new Promise(resolve => chrome.runtime.sendMessage({q: "notifications"},response => resolve(response))) }
...
async function initMain() {
    ...
    initSubscriptions();
    if(url.match(/^https?:\/\/osu\.ppy\.sh\/users\//)){ initSubscribeButton(); }
...
function initSubscriptionsButton() {
    ...
    subscriptionButton.innerHTML = `<button class="nav-button nav-button--stadium js-click-menu" id="subscriptionsbutton">
    ...
    <div class="nav-click-popup js-click-menu"><div id="widgetSubscription" class="notification-popup u-fancy-scrollbar js-nav2--centered-popup hidden" style="transform: translateX(-20px);"><div id="subscriptionsContainer" class="notification-popup__scroll-container"><div class="notification-popup__buttons">
      <button id="switchNotifications" type="button" class="notification-action-button"><span class="notification-action-button__text">Notifications</span></button>
      <div class="notification-popup__clear-button"><button id="switchSubscriptions" type="button" class="notification-action-button"><span class="notification-action-button__text">Subscriptions</span></button></div></div>
    ...
    $("#subscriptionsbutton").click(function() {
        if(!$("#widgetSubscription")[0].classList.contains("hidden")) {
            chrome.runtime.sendMessage({q: "manageNotification", action:"readAll"})
        }
    ...
    if($("button[disabled]").length === 0) {
        ...
        $("#subscribebutton").click(async function(){
            ...
            if(!subscriptions.find(s => s.id === userJSON.id)) {
                chrome.runtime.sendMessage({q: "manageSubscriber", action:"add", userJSON}, response => {
                    if(response.status) {
                    ...
                    } else { alert("An error occured during adding subscriptions.") }
                })
            } else {
                chrome.runtime.sendMessage({q: "manageSubscriber", action:"remove", userJSON}, response => {
                    if(response.status) {
...
function genereateNotificationsContainer() {
    if(notifications.filter(n => !n.read).length > 0) { $('#notificationicon')[0].classList.add("notification-icon--glow") }
    else { $('#notificationicon')[0].classList.remove("notification-icon--glow") }
